...
def gep(argv):
    ...
    '''
    r=dendrogram(linkage(distanceMatrix, method='complete'), color_threshold=0.3, leaf_font_size=7, no_plot=True)
    # plt.show()
    leaves=r['leaves']
	...
	fold_score=[]
		...
		kf = cross_validation.ShuffleSplit(len(yt), n_iter=10, test_size=0.33,random_state=0)
		sp2=0
		sp4=0
		s1_prediction[g]={}
		prediction=[]
		if f=='control':
			...
			pred_tot=[]
			expected=[]
			new_train=clusters[0]
			tot_train=leaves
			for i,l in enumerate(new_train):
				new_train2=new_train[:i]
				#print len(new_train2)
				new_train3=new_train[i+1:]
				#print len(new_train3)
				tot_train2=tot_train[:i]
				tot_train3=tot_train[i+1:]
				new_train2=new_train2+new_train3
				tot_train2=tot_train2+tot_train3
				
				new_test=new_train[i]
				new_train2=np.array(new_train2)
				tot_train2=np.array(tot_train2)
				...
				#print new_test
				#print ""
				cc=max(abs(np.mean(yt[new_train2]) + np.std(yt[new_train2])), abs(np.mean(yt[new_train2]) - np.std(yt[new_train2])))
				eps2=3*np.std(yt[new_train2]) * math.sqrt( math.log(len(yt[new_train])) / len(yt[new_train]) )
		    		svm2 = svm.SVR(C=cc,epsilon=eps2).fit(xt[new_train2],yt[new_train2])
		    		p2=svm2.predict(xt[new_test])
				print p2
				pred_c.append(p2[0])
				
				cc=max(abs(np.mean(yt[tot_train2]) + np.std(yt[tot_train2])), abs(np.mean(yt[tot_train2]) - np.std(yt[tot_train2])))
				eps2=3*np.std(yt[tot_train2]) * math.sqrt( math.log(len(yt[tot_train2])) / len(yt[tot_train2]) )
		    		svm2 = svm.SVR(C=cc,epsilon=eps2).fit(xt[tot_train2],yt[tot_train2])
		    		p2=svm2.predict(xt[new_test])
				pred_tot.append(p2[0])

				expected.append(yt[new_test])
			print pred_c
			print expected
			print "cluster: "+str(spearmanr(np.array(pred_c),np.array(expected))[0])
			print "total: "+str(spearmanr(np.array(pred_tot),np.array(expected))[0])
		
		for traincv, testcv in kf:		
		    xt2=xt[traincv]
		    test=xt[testcv]
		    if f=='control':
		    	selector = SelectPercentile(f_regression, percentile=18).fit(xt[traincv], yt[traincv]) #best
		    	xt2=selector.transform(xt[traincv])
		    	test=selector.transform(xt[testcv])
		    if f=='minmax':
		    	selector = SelectPercentile(f_regression, percentile=18).fit(xt[traincv], yt[traincv]) #best
		    	xt2=selector.transform(xt[traincv])
		    	test=selector.transform(xt[testcv])
		    ...
		    '''
		    if f=='oncogenes':
			xt2=np.array(xt[traincv])
			yt2=np.array(yt[traincv])
			std=np.std(yt2)
			avg=np.mean(yt2)
			ccel=[]
			for i,e in enumerate(yt2):
				if abs(e-avg)>(0.5*std):
					ccel.append(i)
			ccel=np.array(ccel)
			if len(ccel)>=10:
				xt2=xt2[ccel]
				yt2=yt2[ccel]
		    	selector = SelectPercentile(f_regression, percentile=18).fit(xt2, yt2) #best
			#selector = SelectFdr().fit(xt[traincv], yt[traincv])
		    	xt2=selector.transform(xt2)
		    	test=selector.transform(xt[testcv])
		    '''
		    if f=='oncogenes':
			yt2=stats.rankdata(np.array(essent2[g]))
		    	selector = SelectPercentile(f_regression, percentile=18).fit(xt[traincv], yt[traincv]) #best
		    	xt2=selector.transform(xt[traincv])
		    	test=selector.transform(xt[testcv])
		    ...
		    if f=='combined':
		    	selector = SelectPercentile(f_regression, percentile=30).fit(xt[traincv], yt[traincv]) #best
			#selector = SelectFdr().fit(xt[traincv], yt[traincv])
		    	xt2=selector.transform(xt[traincv])
		    	test=selector.transform(xt[testcv])
			
		    eps2=3*np.std(yt[traincv]) * math.sqrt( math.log(len(yt[traincv])) / len(yt[traincv]) )
		    cc=max(abs(np.mean(yt[traincv]) + np.std(yt[traincv])), abs(np.mean(yt[traincv]) - np.std(yt[traincv])))
			
		    
			
		    if f=='oncogenes':

		    	#eps2=3*np.std(yt2[traincv]) * math.sqrt( math.log(len(yt2[traincv])) / len(yt2[traincv]) )
		    	#cc=max(abs(np.mean(yt2[traincv]) + np.std(yt2[traincv])), abs(np.mean(yt2[traincv]) - np.std(yt2[traincv])))
			#eps2=eps2*10000
		    	svm2 = svm.SVR(C=cc,epsilon=eps2).fit(xt2,yt[traincv])
		    	p2=svm2.predict(test)
			p2=stats.rankdata(xt[testcv])
			#print p2
			
		    else:
		    	svm2 = svm.SVR(C=cc,epsilon=eps2).fit(xt[traincv],yt[traincv])
		    	p2=svm2.predict(xt[testcv])
			...
			for cls in clusters:
				...
				new_test=[]
				#print clusters[cls]
				for ccel in clusters[cls]:
					if ccel in traincv:
						new_train.append(ccel)
					if ccel in testcv:
						new_test.append(ccel)
				if len(new_test)>0:
					#print len(new_train),
					#print "\t",
					#print len(new_test),
		    			svm2 = svm.SVR(C=cc,epsilon=eps2).fit(xt[new_train],yt[new_train])
		    			p2=svm2.predict(xt[new_test])
					#print spearmanr(p2,yt[new_test])[0]
					for i,ccel in enumerate(new_test):
						prediction[ccel]=p2[i]
			#print prediction
			p2=prediction.values()
			
		    if f=='minmax':
			testcv=testcv.tolist()
			testcv.sort()	
			#print testcv		
			testcv=np.array(testcv)
			#sp2+=spearmanr(p2,yt[testcv])[0]
		    #else:
		    	#sp2+=spearmanr(p2,yt[testcv])[0]
		    if f=='control':
			fold_score.append(spearmanr(p2,yt[testcv])[0])			
		    #sp4+=pearsonr(p2,yt[testcv])[0]
			
		#if not math.isnan(sp2/10):	
		#   tot_sp1=float(tot_sp1)+float(sp2/10)

        	scores[f]+=float(sp2/10)
		current_score[f]=float(sp2/10)

        	scoresp[f]+=float(sp4/10)
		current_scorep[f]=float(sp4/10)
	b=heapq.nlargest(10 ,current_score, key = lambda k: current_score[k])
        for f in b:
		print "\t"+str(f)+":"+str(current_score[f]),
	b=heapq.nlargest(10 ,current_scorep, key = lambda k: current_scorep[k])
	print ""
        for f in b:
		print "\t"+str(f)+":"+str(current_scorep[f]),
	print ""
	
	print ""
	fold_score=np.array(fold_score)
	if np.mean(fold_score)>0.5:
        	out_file_f1.write(str(g)+"\n")

	if np.mean(fold_score)>0.3 and np.std(fold_score)<0.1:
        	out_file_f2.write(str(g)+"\n")

	if np.mean(fold_score)<-0.05 and np.std(fold_score)<0.1:
        	out_file_f3.write(str(g)+"\n")
	out_file_f1.flush()
	out_file_f2.flush()
	out_file_f3.flush()
        if intr%10==0:
           print str(intr)+" CURRENT AVERAGE =>",

           for f in features:
		if scores[f]/intr:
			print "\t"+str(f)+":"+str(scores[f]/intr),
           print ""
	#yt=np.array(essent2[g])
